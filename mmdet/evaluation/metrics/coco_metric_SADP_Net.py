...
EVAL_CLASS_NAMES = [
    ...
    '25', '26', '27', '28', '38', '37', '36', '35', '34', '33', '32', '31',
    '41', '42', '43', '44', '45', '46', '47', '48'
]
@METRICS.register_module()
class MyCocoMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        self.eval_cat_indices = None

    def _init_eval_categories(self):
        class_names = self.dataset_meta['classes']
        self.eval_cat_indices = [
            i for i, name in enumerate(class_names)
            if name in EVAL_CLASS_NAMES
        ]

        if self._coco_api is not None:
            self.eval_cat_ids = [
                self._coco_api.getCatIds(catNms=[name])[0]
                for name in EVAL_CLASS_NAMES
            ]
        else:
            self.eval_cat_ids = [self.cat_ids[i] for i in self.eval_cat_indices]
    ...
    def results2json(self, results: Sequence[dict],
        ...
        if self.eval_cat_indices is None:
            self._init_eval_categories()


        eval_class_mask = np.isin(
            np.arange(len(self.dataset_meta['classes'])),
            self.eval_cat_indices
        )
        ...
        for idx, result in enumerate(results):
            ...
            valid_mask = eval_class_mask[labels]


            labels = labels[valid_mask]
            bboxes = result['bboxes'][valid_mask]
            scores = result['scores'][valid_mask]
            ...
            masks = [result['masks'][i] for i in np.where(valid_mask)[0]]
        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                if self.classwise:
                    ...
                    for cat_id in self.eval_cat_ids:
                        idx = self.eval_cat_ids.index(cat_id)
                        t = []
